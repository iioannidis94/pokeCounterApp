...
(function teamBootstrap() {
    ...
    // --- ΕΞΕΛΙΓΜΕΝΟΣ ΑΛΓΟΡΙΘΜΟΣ ΑΥΤΟΜΑΤΗΣ ΕΠΙΛΟΓΗΣ (AI) ---
    function autoRecommendTeam() {
        ...
        // 1. Επίσημος Μαθηματικός Τύπος Υπολογισμού Stats
        const getRealStat = (base, iv, ev, level, isHP, natureMult) => {
            base = base || 80; // Fallback αν λείπουν τα base stats
            iv = (iv === '' || iv === undefined) ? 31 : parseInt(iv); // Κενό IV = 31 (Perfect)
        ...
        const getNatureMultiplier = (nature, statName) => {
            ...
            if (!effects) return 1;
            if (effects[0] === statName) return 1.1; // Boost
            if (effects[1] === statName) return 0.9; // Drop
            return 1;
        };

        // 2. Ανάλυση Ρόλου & Συνολικής Δύναμης (Power Level)
        const getRoleDetails = (slot, p) => {
            // Αν υπάρχουν τα BASE_STATS στο data.js τα παίρνει, αλλιώς υποθέτει ένα μέσο όρο (80)
            ...
            let bstReal = rHP + rAtk + rDef + rSpa + rSpd + rSpe; // Συνολικό Power Level
            let bulk = rHP + rDef + rSpd; // Πόσο "ξύλο" αντέχει
        ...
        // 3. Αξιολόγηση και Χτίσιμο Ομάδας
        while (bestTeam.length < 6 && bestTeam.length < pool.length) {
            ...
            pool.filter(x => !bestTeam.includes(x)).forEach(candidate => {
                ...
                // --- A. Βασική Δύναμη (Stats & Level) ---
                score += (details.bstReal / 40); // Όσο μεγαλύτερο level & stats, τόσο πιο ψηλά σκοράρει

                // --- B. Ποιότητα Build (Items, Natures, Moves) ---
                ...
                score += validMoves.length * 10; // Έως +40 πόντοι αν έχει 4 κινήσεις

                // Αν είναι το πρώτο Pokémon, απλά βάλε το πιο δυνατό
                if (bestTeam.length === 0) {
                    if (score > bestScore) { bestScore = score; bestCandidate = candidate; }
                    return;
                }

                // --- C. Αμυντική Συνέργεια (Defensive Synergy) ---
                let teamWeaknesses = {};
                ...
                AT.forEach(t => {
                    ...
                    if (teamWeaknesses[t] > 0) { // Η ομάδα πονάει σε αυτό το Type!
                        if (cMult < 1) score += 45; // Αυτό το Pokémon το κάνει Resist! (Σωτήρας)
                        if (cMult === 0) score += 75; // Είναι Immune! (Θεός)
                        if (cMult > 1) score -= 60; // Πονάει κι αυτό x2, ΜΑΚΡΙΑ!
                    }
                });

                // --- D. Επιθετική Συνέργεια (Move Coverage) ---
                let teamMoveTypes = new Set(bestTeam.flatMap(m => m.slot.moves).filter(x => x));
                validMoves.forEach(mt => {
                    if (!teamMoveTypes.has(mt)) score += 25; // Έχει επίθεση που λείπει από την ομάδα
                });

                // --- E. Ισορροπία Ρόλων (Team Balance) ---
                ...
                if (details.role === 'tank' && tanks < 2) score += 35; // Χρειαζόμαστε άμυνα
                if (details.role === 'physical' && phys < 2) score += 35; // Χρειαζόμαστε Physical DPS
                if (details.role === 'special' && spec < 2) score += 35; // Χρειαζόμαστε Special DPS
                
                if (details.role === 'physical' && phys >= 3) score -= 45; // Γεμίσαμε Physical
                if (details.role === 'special' && spec >= 3) score -= 45;  // Γεμίσαμε Special
                if (details.role === 'tank' && tanks >= 3) score -= 40; // Γεμίσαμε Tanks, δεν έχουμε damage
